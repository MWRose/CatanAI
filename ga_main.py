...
def ga(env, pop_count, keep, select, steps):
    """main ga function, creates a population of random MCTSPlayer individuals and performs [steps] steps"""
    ...
    for i in range(0, steps):
        ...
        for j in range(0,len(pop)):
            ...
            write_csv(str(i), str(config.get_name()), str(fitness), str(config.get_iterations()), str(seconds), str(turns)) # round, name, fitness, itereations, seconds, turns
...
def evolve_step(pop, new_pop, env, keep, select):
    """Helper function for ga(), keeps [keep] individuals and generates our new crossed-over and mutated children"""
    new_pop.clear()
